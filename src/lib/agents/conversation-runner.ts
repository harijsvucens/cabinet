...
function buildSearchInstructions(): string[] {
  return [
    ...
    "",
    "You MUST search the KB before answering any question about Cabinet's content.",
    "The search API understands meaning, synonyms, and natural language questions.",
    "",
    "CRITICAL: Load the `cabinet-search` skill with the `skill` tool BEFORE making",
    "any search API call. The skill contains the API endpoint, query rules, and",
    "non-negotiable rules (no parallel searches, 2-4 key terms, fallback ladder).",
    "Never call the API directly without loading the skill first.",
    "",
    "TRIGGER: Search when the user asks \"what do we know about X\", \"find me...\",",
    "\"where is...\", \"who is...\", \"tell me about...\", or any question that could",
    "be answered by KB pages (projects, exhibitions, brand, marketing, processes).",
    "",
    "After loading the skill, follow its instructions exactly.",
  ];
}
