...
| 34 | **VRAM footprint on RTX 3060 12GB allows full model offloading** | With 12GB of VRAM, loading the full suite of QMD models (embedding, reranker, and fine-tuned 1.7B query expansion) consumes only ~2GB total VRAM. On CUDA, these models execute nearly instantaneously (<200ms per search or expansion), meaning that downgrading to a generic 0.5B model is a significant loss in structured search quality for zero practical speedup. |
| 35 | **Redundant configuration files cause process-env conflicts** | Standard Next.js builds auto-load `.env.local` at boot, whereas daemon processes run as standalone processes that only source `.cabinet.env` via `loadCabinetEnv()`. Specifying conflicting variables (like different `QMD_GENERATE_MODEL` paths) in both files causes mismatched model targets between Next.js and the Daemon. Deleting QMD variables from `.env.local` establishes `.cabinet.env` as the clean single source of truth. |
